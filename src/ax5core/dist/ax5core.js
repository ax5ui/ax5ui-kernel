        ...
        * });
        ...
        function require(mods, callBack, errorBack) {
            var head = doc.head || doc.getElementsByTagName("head")[0],
                ...
                loadCount = mods.length,
                loadErrors = [],
                loadedSrc = {},
                onloadTimer,
                onerrorTimer,
                returned = false,
                scripts = dom.get("script[src]"),
                styles = dom.get("style[href]"),
            ...
            for (var i = 0, l = mods.length; i < l; i++) {
                var src = mods[i],
                    ...
                    hasPlugin = false,
                    plugin,
                    pluginSrc = info.baseUrl + src,
                    attrNm = type === "js" ? "src" : "href",
                    plugLoad,
                    plugErr,
                    s = scripts.length;
                ...
                if (hasPlugin) {
                    ...
                    plugin = type === "js" ? dom.create("script", { type: "text/javascript", src: pluginSrc, "data-src": pluginSrc }) : dom.create("link", { rel: "stylesheet", type: "text/css", href: pluginSrc });

                    plugLoad = function plugLoad(e, pluginSrc) {
                    ...
                    ax5.xhr({
                        ...
                        res: function res(response, status) {
                            var timeId,
                                hasPlugin = false,
                                scripts = dom.get("script[src]"),
                                s = scripts.length;
