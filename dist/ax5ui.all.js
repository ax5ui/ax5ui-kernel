...
newOptions=[];elementOptions.forEach(function(O,OIndex){var option={};option[cfg.columnKeys.optionValue]=O.value;option[cfg.columnKeys.optionText]=O.text;option[cfg.columnKeys.optionSelected]=O.selected;option['@index']=OIndex;if(O.selected)setSelected.call(self,queIdx,option);newOptions.push(option);option=null;});item.options=newOptions;}if(!item.multiple&&item.selected.length==0){item.selected=jQuery.extend({},item.options[0]);}po=null;elementOptions=null;newOptions=null;return item.options;};}(),getQueIdx=function getQueIdx(boundID){if(!U.isString(boundID))boundID=jQuery(boundID).data("ax5-select");if(!U.isString(boundID)){console.log(ax5.info.getError("ax5select","402","val"));return;}return U.search(this.queue,function(){return this.id==boundID;});}; /// private end
/**
         ...
         * ```
         * ```
         */this.init=function(){this.onStateChanged=cfg.onStateChanged;jQuery(window).bind("resize.ax5select-display",function(){alignSelectDisplay.call(this);}.bind(this));}; /**
         * bind select
...
(function(data){if(U.isObject(data)&&!data.error){item=jQuery.extend(true,item,data);}})(U.parseJson(item.$target.attr("data-ax5select-config"),true));queIdx=U.search(this.queue,function(){return this.id==item.id;});if(queIdx===-1){this.queue.push(item);bindSelectTarget.call(this,this.queue.length-1);}else {this.queue[queIdx]=jQuery.extend({},this.queue[queIdx],item);bindSelectTarget.call(this,queIdx);}selectConfig=null;queIdx=null;return this;}; /**
         ...
         * @returns {ax5.ui.select}
         */this.open=function(){return function(queIdx,tryCount){ /**
                 * open select from the outside
                 */if(queIdx instanceof jQuery||U.isElement(queIdx)){var select_id=jQuery(queIdx).data("ax5-select");queIdx=ax5.util.search(this.queue,function(){return this.id==select_id;});if(queIdx==-1){console.log(ax5.info.getError("ax5select","402","open"));return this;}}var item=this.queue[queIdx];var data={},focusTop,selectedOptionEl;if(item.$display.attr("disabled"))return this;if(this.openTimer)clearTimeout(this.openTimer);if(this.activeSelectOptionGroup){if(this.activeSelectQueueIndex==queIdx){return this;}if(tryCount>2)return this;this.close();this.openTimer=setTimeout(function(){this.open(queIdx,(tryCount||0)+1);}.bind(this),cfg.animateTime);return this;} /// 템플릿에 전달할 오브젝트 선언
...
var processor={'index':function index(queIdx,value){ // 옵션선택 초기화
if(!this.queue[queIdx].multiple){this.queue[queIdx].options.forEach(function(n){n.selected=false;});}var getSelected=function getSelected(_item,o){return _item.multiple?!o:true;};if(U.isArray(value.index)){value.index.forEach(function(n){self.queue[queIdx].options[n].selected=getSelected(self.queue[queIdx],self.queue[queIdx].options[n].selected);self.activeSelectOptionGroup.find('[data-option-index="'+n+'"]').attr("data-option-selected",self.queue[queIdx].options[n].selected.toString());});}else {self.queue[queIdx].options[value.index].selected=getSelected(self.queue[queIdx],self.queue[queIdx].options[value.index].selected);self.activeSelectOptionGroup.find('[data-option-index="'+value.index+'"]').attr("data-option-selected",self.queue[queIdx].options[value.index].selected.toString());}syncSelectOptions.call(this,queIdx,this.queue[queIdx].options);this.queue[queIdx].$display.find('[data-ax5-select-display="label"]').html(getLabel.call(this,queIdx));alignSelectOptionGroup.call(this);},'text':function text(queIdx,value){},'arr':function arr(queIdx,value){},'value':function value(queIdx,_value){console.log(queIdx,_value); // todo ~~~
}};return function(boundID,value){var queIdx=getQueIdx.call(this,boundID);if(typeof value=="undefined"){return this.queue[queIdx].selected;}else if(U.isArray(value)){processor.arr.call(this,queIdx,value);}else if(U.isString(value)||U.isNumber(value)){processor.value.call(this,queIdx,value);}else {for(var key in processor){if(value[key]){processor[key].call(this,queIdx,value);break;}}}boundID=null;return this;};}(); /**
         * @method ax5.ui.select.close
         * @returns {ax5.ui.select}
         */this.close=function(boundID,item){if(this.closeTimer)clearTimeout(this.closeTimer);if(!this.activeSelectOptionGroup)return this;item=this.queue[this.activeSelectQueueIndex];item.$display.removeAttr("data-select-option-group-opened");this.activeSelectOptionGroup.addClass("destroy");jQuery(window).unbind("resize.ax5select");jQuery(window).unbind("click.ax5select");jQuery(window).unbind("keyup.ax5select");this.closeTimer=setTimeout(function(){if(this.activeSelectOptionGroup)this.activeSelectOptionGroup.remove();this.activeSelectOptionGroup=null;this.activeSelectQueueIndex=-1;onStateChanged.call(this,item,{self:this,state:"close"});}.bind(this),cfg.animateTime);return this;};this.enable=function(boundID){var queIdx=getQueIdx.call(this,boundID);this.queue[queIdx].$display.removeAttr("disabled");this.queue[queIdx].$select.removeAttr("disabled");return this;};this.disable=function(boundID){var queIdx=getQueIdx.call(this,boundID);this.queue[queIdx].$display.attr("disabled","disabled");this.queue[queIdx].$select.attr("disabled","disabled");return this;}; // 클래스 생성자
...
return axClass;}(); // ax5.ui에 연결
})(ax5.ui,ax5.ui.root);ax5.ui.select_instance=new ax5.ui.select();jQuery.fn.ax5select=function(){return function(config){if(ax5.util.isString(arguments[0])){var methodName=arguments[0],arg=arguments[1];switch(methodName){case "setValue":ax5.ui.select_instance.val(this,arg);break;case "getValue":return ax5.ui.select_instance.val(this);break;default: //
console.log("hh??");return this;}}else {if(typeof config=="undefined")config={};jQuery.each(this,function(){var defaultConfig={target:this};config=jQuery.extend({},config,defaultConfig);ax5.ui.select_instance.bind(config);});}return this;};}(); // ax5.ui.select
(function(root,_SUPER_){ /**
     * @class ax5.ui.select
     * @classdesc
     * @version 0.4.5
